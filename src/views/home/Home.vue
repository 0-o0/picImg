...
export default {
  ...
  methods: {
    ...
    handleWatermarkEnd(e) {
      if (!this.leftTempList) {
        this.leftTempList = [] // 如果 leftTempList 未定义，则初始化为一个空数组
      }
      this.leftTempList.unshift(e) // 现在可以安全地调用 unshift 方法
      this.handletempList(this.leftTempList)
      console.log(this.leftTempList)
    },
    handletempList(tempList) {
      // tempList 处理逻辑
