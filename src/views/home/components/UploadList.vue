      ...
      return new Promise((resolve) => {
        ...
        if (this.svgType === '0') {
          ...
          if (config.strategy_id) {
            formData.append('strategy_id', config.strategy_id)
          }
          if (config.album_id) {
            formData.append('album_id', config.album_id)
          }
          uploadServer(formData).then((res) => {
            console.log(res)
            this.svgType = res.data.data.status ? '3' : '2'
            this.linkUrl = res.data.data.imgurl
            resolve(res.data)
          })
        } else {
          resolve({ msg: '当前图片已上传' })
